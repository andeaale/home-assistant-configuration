...
Advantages:
- Works with all Plex Clients (media_player.plex adds players with incorrect
names like 'plex web safari' and never displays them)
...
- No media control (i.e. play, pause, volume) yet

So this is my crappy way to see Plex clients connected to my Plex server.
If you make this better, please share with me
...
def get_plexclients(plex_server_url):
    ...
    response = requests.get(clients_url, verify=False)
